...
name: hgs-plugin-dev
description: >
  Develop plugins for HyprGlassShell (HGS), a QML-based Linux desktop shell built on
  ...
  to create, modify, or debug a HGS plugin, or asks about the HGS plugin API.
compatibility: Designed for Claude Code (or similar products)
metadata:
  author: HyprGlassShell
  version: "1.0"
  domain: qml-desktop-development
  framework: HyprGlassShell
  languages: qml, javascript
allowed-tools: Bash Read Write Edit
---

# HyprGlassShell Plugin Development

## Overview

HGS plugins extend the desktop shell with custom widgets, background services, launcher
integrations, and desktop widgets. Plugins are QML components discovered from
`~/.config/HyprGlassShell/plugins/`.

**Minimum plugin structure:**

```
~/.config/HyprGlassShell/plugins/YourPlugin/
...
**Plugin registry:** Community plugins are available at https://plugins.coastlinesec.com/
...
- **Shows in the bar?** - Use `widget`. Displays a pill in HGSBar, optionally opens a popout,
...
- **Provides searchable/actionable items?** - Use `launcher`. Items appear in the HGS launcher
...
Item {
    ...
    function executeItem(item) {
        ...
        else if (type === "copy") Quickshell.execDetached(["hgs", "cl", "copy", data])
    }
}
```
...
**Common widgets from `qs.Widgets`:** `StyledText`, `StyledRect`, `HGSIcon`, `HGSButton`, `HGSToggle`, `HGSTextField`, `HGSSlider`, `HGSGridView`, `CachingImage`.
...
Quickshell.execDetached(["hgs", "cl", "copy", textToCopy])
```
...
2. Run the shell with verbose output: `qs -v -p $CONFIGPATH/quickshell/hgs/shell.qml`
3. Open Settings > Plugins > Scan for Plugins
4. Enable your plugin and add it to the HGSBar layout

**Common issues:**
- Plugin not detected: check plugin.json syntax with `jq . plugin.json`
- Widget not showing: ensure it's enabled AND added to a HGSBar section
...
5. **Using `globalThis.clipboard`** - Does not exist; use `Quickshell.execDetached(["hgs", "cl", "copy", text])`
