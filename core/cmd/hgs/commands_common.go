...
import (
	...
	"github.com/CoastLineSec/HyprGlassShell/core/internal/server"
...
var runCmd = &cobra.Command{
	Use:     "run",
	Short:   "Launch quickshell with HGS configuration",
	Long:    "Launch quickshell with HGS configuration (qs -c hgs)",
	PreRunE: findConfig,
	Run: func(cmd *cobra.Command, args []string) {
		daemon, _ := cmd.Flags().GetBool("daemon")
		session, _ := cmd.Flags().GetBool("session")
		if v, _ := cmd.Flags().GetString("log-level"); v != "" {
			if err := os.Setenv("HGS_LOG_LEVEL", v); err != nil {
				log.Fatalf("Failed to set HGS_LOG_LEVEL: %v", err)
			}
		}
		if v, _ := cmd.Flags().GetString("log-file"); v != "" {
			if err := os.Setenv("HGS_LOG_FILE", v); err != nil {
				log.Fatalf("Failed to set HGS_LOG_FILE: %v", err)
			}
		}
...
var restartCmd = &cobra.Command{
	Use:     "restart",
	Short:   "Restart quickshell with HGS configuration",
	Long:    "Kill existing HGS shell processes and restart quickshell with HGS configuration",
	PreRunE: findConfig,
	Run: func(cmd *cobra.Command, args []string) {
		restartShell()
	},
...
var killCmd = &cobra.Command{
	Use:   "kill",
	Short: "Kill running HGS shell processes",
	Long:  "Kill all running quickshell processes with HGS configuration",
	Run: func(cmd *cobra.Command, args []string) {
		killShell()
	},
}
...
var ipcCmd = &cobra.Command{
	Use:   "ipc",
	Short: "Send IPC commands to running HGS shell",
	Long: `Send IPC commands to the running HGS shell.

  hgs ipc call <target> <function> [args...]   invoke a command
  hgs ipc list                                 list all targets and functions

Full reference: https://coastlinesec.com/docs/hyprglassshell/keybinds-ipc`,
...
var debugSrvCmd = &cobra.Command{
	...
	Long:  "Start the Unix socket debug server for HGS",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsCmd = &cobra.Command{
	Use:   "plugins",
	Short: "Manage HGS plugins",
	Long:  "Browse and manage HGS plugins from the registry",
}

var pluginsBrowseCmd = &cobra.Command{
	Use:   "browse",
	Short: "Browse available plugins",
	Long:  "Browse available plugins from the HGS plugin registry",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsListCmd = &cobra.Command{
	...
	Long:  "List all installed HGS plugins",
	Run: func(cmd *cobra.Command, args []string) {
...
var pluginsInstallCmd = &cobra.Command{
	...
	Long:  "Install a HGS plugin from the registry using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
...
var pluginsUninstallCmd = &cobra.Command{
	...
	Long:  "Uninstall a HGS plugin using its ID (e.g., 'myPlugin'). Plugin names with spaces are also supported for backward compatibility.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
var pluginsUpdateCmd = &cobra.Command{
	...
	Long:  "Update an installed HGS plugin using its ID (e.g., 'myPlugin'). Plugin names are also supported.",
	Args:  cobra.ExactArgs(1),
	ValidArgsFunction: func(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
		if len(args) != 0 {
			return nil, cobra.ShellCompDirectiveNoFileComp
...
// Git builds: hgs (git) v0.6.2-XXXX
// Stable releases: hgs v0.6.2
func formatVersion(version string) string {
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("hgs (git) v%s-%s", matches[1], matches[2])
	}
	...
	if matches := re.FindStringSubmatch(version); matches != nil {
		...
		return fmt.Sprintf("hgs (git) v%s-%s", baseVersion, matches[1])
	}

	// Stable release format: 0.6.2
	re = regexp.MustCompile(`^([\d.]+)$`)
	if matches := re.FindStringSubmatch(version); matches != nil {
		return fmt.Sprintf("hgs v%s", matches[1])
	}

	return fmt.Sprintf("hgs %s", version)
}

func getBaseVersion() string {
	paths := []string{
		"/usr/share/quickshell/hgs/VERSION",
		"/usr/local/share/quickshell/hgs/VERSION",
		"/etc/xdg/quickshell/hgs/VERSION",
	}
