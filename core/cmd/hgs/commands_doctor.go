...
import (
	...
	"github.com/CoastLineSec/HyprGlassShell/core/internal/blur"
	"github.com/CoastLineSec/HyprGlassShell/core/internal/clipboard"
	...
	"github.com/CoastLineSec/HyprGlassShell/core/internal/tui"
...
var doctorCmd = &cobra.Command{
	Use:   "doctor",
	Short: "Diagnose HGS installation and dependencies",
	Long:  "Check system health, verify dependencies, and diagnose configuration issues for HGS",
	Run:   runDoctor,
}
...
const (
	...
	doctorDocsURL      = "https://coastlinesec.com/docs/hyprglassshell/cli-doctor"
)
...
func checkSystemInfo() []checkResult {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "Unsupported distribution") {
			...
			case osRelease["PRETTY_NAME"] != "":
				message = fmt.Sprintf("%s (not supported by hgs setup)", osRelease["PRETTY_NAME"])
				details = "HGS may work but automatic installation is not available"
			}
		}
	...
	case xdgSessionType == "x11":
		results = append(results, checkResult{catSystem, "Display Server", statusError, "X11 (HGS requires Wayland)", "", doctorDocsURL + "#display-server"})
	default:
		results = append(results, checkResult{
...
func checkVersions(qsMissingFeatures bool) []checkResult {
	hgsCliPath, _ := os.Executable()
	hgsCliDetails := ""
	if doctorVerbose {
		hgsCliDetails = hgsCliPath
	}

	results := []checkResult{
		{catVersions, "HGS CLI", statusOK, formatVersion(Version), hgsCliDetails, doctorDocsURL + "#hgs-cli"},
	}
	...
	hgsVersion, hgsPath := getHGSShellVersion()
	if hgsVersion != "" {
		results = append(results, checkResult{catVersions, "HGS Shell", statusOK, hgsVersion, hgsPath, doctorDocsURL + "#hgs-shell"})
	} else {
		results = append(results, checkResult{catVersions, "HGS Shell", statusError, "Not installed or not detected", "Run 'hgs setup' to install", doctorDocsURL + "#hgs-shell"})
	}

	return results
}

func getHGSShellVersion() (version, path string) {
	...
	if hgsPath, err := config.LocateHGSConfig(); err == nil {
		versionFile := filepath.Join(hgsPath, "VERSION")
		if data, err := os.ReadFile(versionFile); err == nil {
			return strings.TrimSpace(string(data)), hgsPath
		}
		return "installed", hgsPath
	}
...
func checkHGSInstallation() []checkResult {
	var results []checkResult

	hgsPath := ""
	if err := findConfig(nil, nil); err == nil && configPath != "" {
		hgsPath = configPath
	} else if path, err := config.LocateHGSConfig(); err == nil {
		hgsPath = path
	}

	if hgsPath == "" {
		return []checkResult{{catInstallation, "HGS Configuration", statusError, "Not found", "shell.qml not found in any config path", doctorDocsURL + "#hgs-configuration"}}
	}

	results = append(results, checkResult{catInstallation, "HGS Configuration", statusOK, "Found", hgsPath, doctorDocsURL + "#hgs-configuration"})

	shellQml := filepath.Join(hgsPath, "shell.qml")
	if _, err := os.Stat(shellQml); err != nil {
		results = append(results, checkResult{catInstallation, "shell.qml", statusError, "Missing", shellQml, doctorDocsURL + "#hgs-configuration"})
	} else {
		results = append(results, checkResult{catInstallation, "shell.qml", statusOK, "Present", shellQml, doctorDocsURL + "#hgs-configuration"})
	}

	if doctorVerbose {
		installType := "Unknown"
		switch {
		case strings.Contains(hgsPath, "/nix/store"):
			installType = "Nix store"
		case strings.Contains(hgsPath, ".local/share") || strings.Contains(hgsPath, "/usr/share"):
			installType = "System package"
		case strings.Contains(hgsPath, ".config"):
			installType = "User config"
		}
		results = append(results, checkResult{catInstallation, "Install Type", statusInfo, installType, hgsPath, doctorDocsURL + "#hgs-configuration"})
	}
...
func detectRunningWM() string {
	...
	case os.Getenv("HYPRLAND_INSTANCE_SIGNATURE") != "":
		return "Hyprland"
	case strings.Contains(strings.ToLower(os.Getenv("XDG_CURRENT_DESKTOP")), "hyprland"):
...
`
	...
	features := []struct{ name, desc string }{
		...
		{"ShortcutInhibitor", "Allow compositor shortcut management"},
...
func checkOptionalDependencies() []checkResult {
	...
	deps := []struct {
		...
		{"hgssearch", "dsearch", "File search", false},
...
func checkConfigurationFiles() []checkResult {
	...
	hgsDir := "HyprGlassShell"

	configFiles := []struct{ name, path string }{
		{"settings.json", filepath.Join(configDir, hgsDir, "settings.json")},
		{"clsettings.json", filepath.Join(configDir, hgsDir, "clsettings.json")},
		{"plugin_settings.json", filepath.Join(configDir, hgsDir, "plugin_settings.json")},
		{"session.json", filepath.Join(utils.XDGStateHome(), hgsDir, "session.json")},
		{"hgs-colors.json", filepath.Join(cacheDir, hgsDir, "hgs-colors.json")},
	}
...
func checkSystemdServices() []checkResult {
	...
	hgsState := getServiceState("hgs", true)
	if !hgsState.exists {
		results = append(results, checkResult{catServices, "hgs.service", statusInfo, "Not installed", "Optional user service", doctorDocsURL + "#services"})
	} else {
		status, message := statusOK, hgsState.enabled
		if hgsState.active != "" {
			message = fmt.Sprintf("%s, %s", hgsState.enabled, hgsState.active)
		}
		switch {
		case hgsState.active == "failed":
			status = statusError
		case hgsState.active == "active":
		case hgsState.enabled == "disabled":
			status, message = statusWarn, "Disabled"
		case hgsState.active == "inactive":
			status = statusError
		}
		results = append(results, checkResult{catServices, "hgs.service", status, message, "", doctorDocsURL + "#services"})
	}
...
func formatResultsPlain(results []checkResult) string {
	...
	sb.WriteString("## HGS Doctor Report\n\n")
...
func checkFonts() []checkResult {
	...
	settingsPath := filepath.Join(configDir, "HyprGlassShell", "settings.json")
