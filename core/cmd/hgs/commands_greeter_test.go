...
func TestSyncGreeterConfigsAndAuthDelegatesSharedAuth(t *testing.T) {
	...
	var calls []string
	greeterConfigSyncFn = func(hgsPath, compositor string, logFunc func(string), sudoPassword string) error {
		if hgsPath != "/tmp/hgs" {
			t.Fatalf("unexpected hgsPath %q", hgsPath)
		}
	...
	err := syncGreeterConfigsAndAuth("/tmp/hgs", "hyprland", func(string) {}, sharedpam.SyncAuthOptions{
...
func TestSyncGreeterConfigsAndAuthStopsOnConfigError(t *testing.T) {
	...
	err := syncGreeterConfigsAndAuth("/tmp/hgs", "hyprland", func(string) {}, sharedpam.SyncAuthOptions{}, nil)
