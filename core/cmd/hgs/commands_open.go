...
var openCmd = &cobra.Command{
	...
	Long: `Open a target (URL, file, or other resource) using the HGS application picker.
...
Examples:
  hgs open https://example.com                    # Open URL with browser picker
  hgs open file.pdf                               # Open file (MIME auto-detected)
  hgs open file.pdf --mime application/pdf        # Override MIME detection
  hgs open document.odt --category Office         # Open with office applications`,
...
func runOpen(target string) {
	...
	case isScheme(target, "http://"), isScheme(target, "https://"), isScheme(target, "hgs://"):
	...
	if detectedMimeType == "" && len(openCategories) == 0 && (isScheme(target, "http://") || isScheme(target, "https://") || isScheme(target, "hgs://")) {
		method = "browser.open"
		params["url"] = target
	}
	...
	if err := sendServerRequestFireAndForget(req); err != nil {
		fmt.Println("HGS is not running. Please start HGS first.")
		os.Exit(1)
	}
