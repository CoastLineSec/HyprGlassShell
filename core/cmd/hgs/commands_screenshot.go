...
import (
	...
	"github.com/CoastLineSec/HyprGlassShell/core/internal/screenshot"
...
Modes:
  ...
  window      - Capture the focused window (Hyprland)
...
Examples:
  hgs screenshot                     # Region select, save file + clipboard
  hgs screenshot full                # Full screen of focused output
  hgs screenshot all                 # All screens combined
  hgs screenshot output -o DP-1      # Specific output
  hgs screenshot window              # Focused window (Hyprland)
  hgs screenshot last                # Last region (pre-selected)
  hgs screenshot --reset             # Reset last region pre-selection
  hgs screenshot --no-clipboard      # Save file only
  hgs screenshot --no-file           # Clipboard only
  hgs screenshot --no-confirm        # Region capture on mouse release
  hgs screenshot --cursor=on         # Include cursor
  hgs screenshot -f jpg -q 85        # JPEG with quality 85`,
}
...
var ssWindowCmd = &cobra.Command{
	...
	Long:  `Capture the currently focused window. Supported on Hyprland.`,
