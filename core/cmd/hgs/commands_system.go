...
var systemCmd = &cobra.Command{
	...
	Long:  "System-level operations (updates, etc.). Runs against installed package managers directly; does not require the HGS server.",
}
...
Examples:
  hgs system update --check                  # list available updates
  hgs system update                          # apply updates (interactive prompt)
  hgs system update --noconfirm              # apply updates without prompting
  hgs system update --dry                    # simulate without changing anything
  hgs system update --no-flatpak --noconfirm # apply system updates only
  hgs system update --interval 3600          # set the server poll interval to 1h`,
	Run: runSystemUpdate,
}
...
func init() {
	...
	systemUpdateCmd.Flags().IntVar(&sysUpdateIntervalS, "interval", -1, "Set the HGS server poll interval in seconds and exit (requires running server)")
...
func runSystemUpdateSetInterval(seconds int) {
	...
	if err != nil {
		log.Fatalf("Failed: %v (is hgs server running?)", err)
	}
	if resp.Error != "" {
