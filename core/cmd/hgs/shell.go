...
func appendLogEnv(env []string) []string {
	if v := os.Getenv("HGS_LOG_LEVEL"); v != "" {
		env = append(env, "HGS_LOG_LEVEL="+v)
	}
	if v := os.Getenv("HGS_LOG_FILE"); v != "" {
		env = append(env, "HGS_LOG_FILE="+v)
	}
	return env
}
...
func getPIDFilePath() string {
	return filepath.Join(getRuntimeDir(), fmt.Sprintf("coastlinesec-%d.pid", os.Getpid()))
}
...
func getAllHGSPIDs() []int {
	...
	for _, entry := range entries {
		if !strings.HasPrefix(entry.Name(), "coastlinesec-") || !strings.HasSuffix(entry.Name(), ".pid") {
			continue
		}
		...
		parentPIDStr := strings.TrimPrefix(entry.Name(), "coastlinesec-")
...
func runShellInteractive(session bool) {
	...
	cmd.Env = append(os.Environ(), "HGS_SOCKET="+socketPath)
	...
	if isSessionManaged && hasSystemdRun() {
		cmd.Env = append(cmd.Env, "HGS_DEFAULT_LAUNCH_PREFIX=systemd-run --user --scope")
	}

	homeDir, err := os.UserHomeDir()
	if err == nil && os.Getenv("HGS_DISABLE_HOT_RELOAD") == "" {
		if !strings.HasPrefix(configPath, homeDir) {
			cmd.Env = append(cmd.Env, "HGS_DISABLE_HOT_RELOAD=1")
		}
	}
...
func restartShell() {
	...
	if len(pids) == 0 {
		log.Info("No running HGS shell instances found. Starting daemon...")
		runShellDaemon(false)
		return
	}
	...
	for pid := range uniquePids {
		...
		if err := proc.Signal(syscall.SIGUSR1); err != nil {
			...
			log.Infof("Sent SIGUSR1 to HGS process with PID %d", pid)
		}
	}
}
...
func killShell() {
	...
	if len(pids) == 0 {
		log.Info("No running HGS shell instances found.")
		return
	}
	...
	for pid := range uniquePids {
		...
		if err := proc.Kill(); err != nil {
			...
			log.Infof("Killed HGS process with PID %d", pid)
		}
	}
...
func runShellDaemon(session bool) {
	...
	if !isDaemonChild {
		...
		log.Infof("HGS shell daemon started (PID: %d)", cmd.Process.Pid)
		return
	}
...
func printIPCHelp() {
	fmt.Println("Usage: hgs ipc call <target> <function> [args...]")
	fmt.Println()
...
func printIPCHelpFailure(err error) {
	...
	fmt.Println()
	fmt.Println("  Full docs:  https://coastlinesec.com/docs/hyprglassshell/keybinds-ipc")
	fmt.Println("  Try:        hgs ipc call <target> <function>")
}
...
func logStartupFailure(startTime time.Time, exitCode int, tracker *stderrTracker) {
	...
	if containsFontCrashSignature(tracker.String()) {
		log.Errorf("HGS startup failed due to a potential font/rendering crash. Try running 'fc-cache -fv' and restarting HGS.")
	} else {
		log.Errorf("HGS startup failed (exit code %d). Run 'hgs doctor' for more diagnostics.", exitCode)
	}
}
