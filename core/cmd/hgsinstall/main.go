...
import (
	...
	"github.com/CoastLineSec/HyprGlassShell/core/internal/headless"
...
var rootCmd = &cobra.Command{
	Use:   "hgsinstall",
	Short: "Install HyprGlassShell and its dependencies",
	Long: `hgsinstall sets up HyprGlassShell with your chosen compositor and terminal.
...
func init() {
	rootCmd.Flags().StringVarP(&compositor, "compositor", "c", "", "Compositor/WM to install: hyprland (enables headless mode)")
	rootCmd.Flags().StringVarP(&term, "term", "t", "", "Terminal emulator to install: ghostty, kitty, or alacritty (enables headless mode)")
	rootCmd.Flags().StringSliceVar(&includeDeps, "include-deps", []string{}, "Optional deps to enable (e.g. hgs-greeter)")
...
func main() {
	if os.Getuid() == 0 {
		fmt.Fprintln(os.Stderr, "Error: hgsinstall must not be run as root")
		os.Exit(1)
	}
...
func runHGSinstall(cmd *cobra.Command, args []string) error {
...
func runHeadless() error {
	...
	if compositor == "" {
		return fmt.Errorf("--compositor is required for headless mode (hyprland)")
	}
	if term == "" {
