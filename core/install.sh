...
LATEST_VERSION=$(curl -s https://api.github.com/repos/CoastLineSec/HyprGlassShell/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
printf "%bInstalling HGSinstall %s for %s...%b\n" "$GREEN" "$LATEST_VERSION" "$ARCH" "$NC"
...
curl -L "https://github.com/CoastLineSec/HyprGlassShell/releases/download/$LATEST_VERSION/hgsinstall-$ARCH.gz" -o "installer.gz"
curl -L "https://github.com/CoastLineSec/HyprGlassShell/releases/download/$LATEST_VERSION/hgsinstall-$ARCH.gz.sha256" -o "expected.sha256"
