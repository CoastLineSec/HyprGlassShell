...
func (f *FedoraDistribution) DetectDependenciesWithTerminal(ctx context.Context, wm deps.WindowManager, terminal deps.Terminal) ([]deps.Dependency, error) {
	...
	dependencies = append(dependencies, f.detectHGS())
	...
	dependencies = append(dependencies, f.detectHGSGreeter())
...
func (f *FedoraDistribution) GetPackageMappingWithVariants(wm deps.WindowManager, variants map[string]deps.PackageVariant) map[string]PackageMapping {
	packages := map[string]PackageMapping{
		...
		"ghostty":                {Name: "ghostty", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"},
		...
		"hgs-greeter":          {Name: "hgs-greeter", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"},
		"matugen":              {Name: "matugen", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"},
		"hgs (HyprGlassShell)": f.getHgsMapping(variants["hgs (HyprGlassShell)"]),
		"dgop":                 {Name: "dgop", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"},
	}
...
func (f *FedoraDistribution) getQuickshellMapping(variant deps.PackageVariant) PackageMapping {
	if forceQuickshellGit || variant == deps.VariantGit {
		return PackageMapping{Name: "quickshell-git", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"}
	}
	return PackageMapping{Name: "quickshell", Repository: RepoTypeCOPR, RepoURL: "avengemedia/coastlinesec"}
}

func (f *FedoraDistribution) getHgsMapping(variant deps.PackageVariant) PackageMapping {
	if variant == deps.VariantGit {
		return PackageMapping{Name: "hgs", Repository: RepoTypeCOPR, RepoURL: "avengemedia/hgs-git"}
	}
	return PackageMapping{Name: "hgs", Repository: RepoTypeCOPR, RepoURL: "avengemedia/hgs"}
}
...
func (f *FedoraDistribution) detectHGSGreeter() deps.Dependency {
	return f.detectOptionalPackage("hgs-greeter", "HyprGlassShell greetd greeter", f.packageInstalled("hgs-greeter"))
}
...
func (f *FedoraDistribution) InstallPackages(ctx context.Context, dependencies []deps.Dependency, wm deps.WindowManager, sudoPassword string, reinstallFlags map[string]bool, disabledFlags map[string]bool, skipGlobalUseFlags bool, progressChan chan<- InstallProgressMsg) error {
	...
	if err := f.EnableHGSService(ctx, wm); err != nil {
		f.log(fmt.Sprintf("Warning: failed to enable hgs service: %v", err))
	}
