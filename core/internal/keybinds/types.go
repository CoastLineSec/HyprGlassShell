...
type Keybind struct {
	...
	HasDefault      bool     `json:"hasDefault,omitempty"` // override has a HGS default to revert to
}

type HGSBindsStatus struct {
	Exists          bool   `json:"exists"`
	Included        bool   `json:"included"`
	IncludePosition int    `json:"includePosition"`
	TotalIncludes   int    `json:"totalIncludes"`
	BindsAfterHGS   int    `json:"bindsAfterHgs"`
...
type CheatSheet struct {
	...
	HGSBindsIncluded bool                 `json:"hgsBindsIncluded"`
	HGSStatus        *HGSBindsStatus      `json:"hgsStatus,omitempty"`
}
...
type WritableProvider interface {
	...
	// hgs/user-keybinds.lua; single-file providers delete the line.
	RemoveBind(key string) error
	// ResetBind reverts a user override to its HGS default. On single-file
