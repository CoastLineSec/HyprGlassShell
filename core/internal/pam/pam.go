...
const (
	GreeterPamManagedBlockStart = "# BEGIN HGS GREETER AUTH (managed by hgs greeter sync)"
	GreeterPamManagedBlockEnd   = "# END HGS GREETER AUTH"

	LockscreenPamManagedBlockStart = "# BEGIN HGS LOCKSCREEN AUTH (managed by hgs greeter sync)"
	LockscreenPamManagedBlockEnd   = "# END HGS LOCKSCREEN AUTH"

	LockscreenU2FPamManagedBlockStart = "# BEGIN HGS LOCKSCREEN U2F AUTH (managed by hgs auth sync)"
	LockscreenU2FPamManagedBlockEnd   = "# END HGS LOCKSCREEN U2F AUTH"

	legacyGreeterPamFprintComment = "# HGS greeter fingerprint"
	legacyGreeterPamU2FComment    = "# HGS greeter U2F"

	GreetdPamPath      = "/etc/pam.d/greetd"
	HGSshellPamPath    = "/etc/pam.d/hgsshell"
	HGSshellU2FPamPath = "/etc/pam.d/hgsshell-u2f"
)
...
func defaultSyncDeps() syncDeps {
	return syncDeps{
		...
		hgsshellPath:    HGSshellPamPath,
		hgsshellU2fPath: HGSshellU2FPamPath,
		isNixOS:         IsNixOS,
...
func removeManagedGreeterPamBlockWithDeps(logFunc func(string), sudoPassword string, deps syncDeps) error {
	...
	logFunc("✓ Removed HGS managed PAM block from " + deps.greetdPath)
	return nil
}
...
func syncLockscreenPamConfigWithDeps(logFunc func(string), sudoPassword string, deps syncDeps) error {
	if deps.isNixOS() {
		logFunc("ℹ NixOS detected. HGS continues to use /etc/pam.d/login for lock screen password auth on NixOS unless you declare security.pam.services.hgsshell yourself. U2F and fingerprint are handled separately and should not be included in hgsshell.")
		return nil
	}

	existingData, err := deps.readFile(deps.hgsshellPath)
	if err == nil {
		if !hasManagedLockscreenPamFile(string(existingData)) {
			logFunc("ℹ Custom /etc/pam.d/hgsshell found (no HGS block). Skipping.")
			return nil
		}
	} else if !os.IsNotExist(err) {
		return fmt.Errorf("failed to read %s: %w", deps.hgsshellPath, err)
	}

	content, err := buildManagedLockscreenPamContent(deps.pamDir, deps.readFile)
	if err != nil {
		return fmt.Errorf("failed to build %s from %s: %w", deps.hgsshellPath, filepath.Join(deps.pamDir, "login"), err)
	}

	if err := writeManagedPamFile(content, deps.hgsshellPath, sudoPassword, deps); err != nil {
		return fmt.Errorf("failed to write %s: %w", deps.hgsshellPath, err)
	}

	logFunc("✓ Created or updated /etc/pam.d/hgsshell for lock screen authentication")
	return nil
}

func syncLockscreenU2FPamConfigWithDeps(logFunc func(string), sudoPassword string, enabled bool, deps syncDeps) error {
	if deps.isNixOS() {
		logFunc("ℹ NixOS detected. HGS does not manage /etc/pam.d/hgsshell-u2f on NixOS. Keep using the bundled U2F helper or configure a custom PAM service yourself.")
		return nil
	}

	existingData, err := deps.readFile(deps.hgsshellU2fPath)
	if err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("failed to read %s: %w", deps.hgsshellU2fPath, err)
	}

	if enabled {
		if err == nil && !hasManagedLockscreenU2FPamFile(string(existingData)) {
			logFunc("ℹ Custom /etc/pam.d/hgsshell-u2f found (no HGS block). Skipping.")
			return nil
		}
		if err := writeManagedPamFile(buildManagedLockscreenU2FPamContent(), deps.hgsshellU2fPath, sudoPassword, deps); err != nil {
			return fmt.Errorf("failed to write %s: %w", deps.hgsshellU2fPath, err)
		}
		logFunc("✓ Created or updated /etc/pam.d/hgsshell-u2f for lock screen security-key authentication")
		return nil
	}
	...
	if err == nil && !hasManagedLockscreenU2FPamFile(string(existingData)) {
		logFunc("ℹ Custom /etc/pam.d/hgsshell-u2f found (no HGS block). Leaving it untouched.")
		return nil
	}

	if err := deps.runSudoCmd(sudoPassword, "rm", "-f", deps.hgsshellU2fPath); err != nil {
		return fmt.Errorf("failed to remove %s: %w", deps.hgsshellU2fPath, err)
	}
	logFunc("✓ Removed HGS-managed /etc/pam.d/hgsshell-u2f")
	return nil
}
...
func syncGreeterPamConfigWithDeps(logFunc func(string), sudoPassword string, settings AuthSettings, forceAuth bool, deps syncDeps) error {
	...
	if deps.isNixOS() {
		logFunc("ℹ NixOS detected: PAM config is managed by NixOS modules. Skipping HGS PAM block write.")
	...
	if wantFprint && includedFprintFile != "" {
		logFunc("⚠ pam_fprintd already present in included " + includedFprintFile + " (managed by authselect/pam-auth-update). Skipping HGS fprint block to avoid double-fingerprint auth.")
		wantFprint = false
	}
	if wantU2f && includedU2fFile != "" {
		logFunc("⚠ pam_u2f already present in included " + includedU2fFile + " (managed by authselect/pam-auth-update). Skipping HGS U2F block to avoid double security-key auth.")
		wantU2f = false
	}
	if !wantFprint && includedFprintFile != "" {
		if fprintToggleEnabled {
			...
			if fprintAvailableForCurrentUser {
				logFunc("  HGS toggle is enabled, and effective auth is provided by the included PAM stack.")
			} else {
				logFunc("  No enrolled fingerprints detected for the current user; password auth remains the effective path.")
			}
		} else {
			if fprintAvailableForCurrentUser {
				logFunc("ℹ Fingerprint auth is active via included " + includedFprintFile + " while HGS fingerprint toggle is off.")
	...
	if !wantU2f && includedU2fFile != "" {
		if u2fToggleEnabled {
			...
			logFunc("  HGS toggle is enabled, but effective auth is provided by the included PAM stack.")
		} else {
			logFunc("⚠ Security-key auth is active via included " + includedU2fFile + " while HGS security-key toggle is off.")
