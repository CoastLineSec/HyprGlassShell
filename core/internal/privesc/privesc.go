...
// EnvOverride returns the tool selected by the $HGS_PRIVESC env var (if any)
...
// than one is installed and $HGS_PRIVESC is not set. If stdin is not a TTY,
