...
Release:        1%{?dist}
Epoch:          2
Summary:        HyprGlassShell - Material 3 inspired shell (git nightly)
...
and optimized for the Hyprland compositor.

This git version tracks the master branch and includes the latest features
and fixes. Includes pre-built hgs CLI binary and QML shell files.

%prep
%setup -q -n hgs-git-source
...
# Build hgs-cli from source using vendored dependencies
...
%posttrans
if [ -d "%{_sysconfdir}/xdg/quickshell/hgs" ]; then
    rmdir "%{_sysconfdir}/xdg/quickshell/hgs" 2>/dev/null || true
