# Spec for HGS Greeter - OpenSUSE/OBS

%global debug_package %{nil}
%global version VERSION_PLACEHOLDER
%global pkg_summary HyprGlassShell greeter for greetd
...
%description
HyprGlassShell greeter for greetd login manager. A modern, Material Design 3
inspired greeter interface built with Quickshell for Wayland compositors.

Targets the Hyprland compositor with automatic configuration. Features session selection, user
...
install -dm755 %{buildroot}%{_datadir}/quickshell/hgs-greeter
cp -r %{_builddir}/hgs-qml/* %{buildroot}%{_datadir}/quickshell/hgs-greeter/

install -Dm755 %{_builddir}/hgs-qml/Modules/Greetd/assets/hgs-greeter %{buildroot}%{_bindir}/hgs-greeter

install -Dm644 %{_builddir}/hgs-qml/Modules/Greetd/README.md %{buildroot}%{_docdir}/hgs-greeter/README.md

install -Dpm0644 %{_builddir}/hgs-qml/systemd/tmpfiles-hgs-greeter.conf %{buildroot}%{_tmpfilesdir}/hgs-greeter.conf

install -Dm644 %{_builddir}/hgs-qml/systemd/sysusers-hgs-greeter.conf %{buildroot}%{_sysusersdir}/hgs-greeter.conf

install -Dm644 %{_builddir}/hgs-qml/LICENSE %{buildroot}%{_docdir}/hgs-greeter/LICENSE

install -dm755 %{buildroot}%{_sharedstatedir}/greeter

# Remove build and development files
rm -rf %{buildroot}%{_datadir}/quickshell/hgs-greeter/.git*
rm -f %{buildroot}%{_datadir}/quickshell/hgs-greeter/.gitignore
rm -rf %{buildroot}%{_datadir}/quickshell/hgs-greeter/.github
rm -rf %{buildroot}%{_datadir}/quickshell/hgs-greeter/distro

%posttrans
if [ -d "%{_sysconfdir}/xdg/quickshell/hgs-greeter" ]; then
    rmdir "%{_sysconfdir}/xdg/quickshell/hgs-greeter" 2>/dev/null || true
    rmdir "%{_sysconfdir}/xdg/quickshell" 2>/dev/null || true
    rmdir "%{_sysconfdir}/xdg" 2>/dev/null || true
fi

%files
%dir %{_docdir}/hgs-greeter
%license %{_docdir}/hgs-greeter/LICENSE
%doc %{_docdir}/hgs-greeter/README.md
%{_bindir}/hgs-greeter
%dir %{_datadir}/quickshell
%{_datadir}/quickshell/hgs-greeter/
%{_tmpfilesdir}/%{name}.conf
%{_sysusersdir}/hgs-greeter.conf
...
if [ -x /usr/sbin/semanage ] && [ -x /usr/sbin/restorecon ]; then
    semanage fcontext -a -t bin_t '%{_bindir}/hgs-greeter' >/dev/null 2>&1 || true
    restorecon %{_bindir}/hgs-greeter >/dev/null 2>&1 || true
    ...
    semanage fcontext -a -t cache_home_t '%{_localstatedir}/cache/hgs-greeter(/.*)?' >/dev/null 2>&1 || true
    restorecon -R %{_localstatedir}/cache/hgs-greeter >/dev/null 2>&1 || true
    semanage fcontext -a -t usr_t '%{_datadir}/quickshell/hgs-greeter(/.*)?' >/dev/null 2>&1 || true
    restorecon -R %{_datadir}/quickshell/hgs-greeter >/dev/null 2>&1 || true
...
chown -R "$GREETER_USER:$GREETER_GROUP" %{_localstatedir}/cache/hgs-greeter 2>/dev/null || true
...
if [ ! -f "$PAM_CONFIG" ]; then
    ...
    if [ "$NEEDS_PAM_UPDATE" -eq 1 ]; then
        cp "$PAM_CONFIG" "$PAM_CONFIG.backup-hgs-greeter"
        write_greetd_pam_config
        chmod 644 "$PAM_CONFIG"
        [ "$1" -eq 1 ] && echo "Updated PAM configuration (old config backed up to $PAM_CONFIG.backup-hgs-greeter)"
    fi
fi
...
command = "/usr/bin/hgs-greeter --command COMPOSITOR_PLACEHOLDER"
GREETD_EOF
...
elif ! grep -q "hgs-greeter" "$GREETD_CONFIG"; then
    if [ -n "$COMPOSITOR" ]; then
        BACKUP_FILE="${GREETD_CONFIG}.backup-$(date +%%Y%%m%%d-%%H%%M%%S)"
        cp "$GREETD_CONFIG" "$BACKUP_FILE" 2>/dev/null || true
        sed -i "/^\[default_session\]/,/^\[/ s|^command =.*|command = \"/usr/bin/hgs-greeter --command $COMPOSITOR\"|" "$GREETD_CONFIG"
        ...
        CONFIG_STATUS="Skipped hgs-greeter command update (no supported compositor detected)"
    fi
fi
...
=========================================================================
        HGS Greeter Installation Complete!
=========================================================================
...
cat << 'EOF'
    ...
    ✓ Greeter directories: /var/cache/hgs-greeter, /var/lib/greeter
...
Documentation: https://coastlinesec.com/docs/hgsgreeter/
=========================================================================
...
if [ "$1" -eq 0 ] && [ -x /usr/sbin/semanage ]; then
    semanage fcontext -d '%{_bindir}/hgs-greeter' 2>/dev/null || true
    semanage fcontext -d '%{_sharedstatedir}/greeter(/.*)?' 2>/dev/null || true
    semanage fcontext -d '%{_localstatedir}/cache/hgs-greeter(/.*)?' 2>/dev/null || true
    semanage fcontext -d '%{_datadir}/quickshell/hgs-greeter(/.*)?' 2>/dev/null || true
fi
