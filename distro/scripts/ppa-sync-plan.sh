#!/usr/bin/env bash
# Build a HGS per-series upload plan by comparing Git/GitHub with Launchpad.
...
PACKAGES=(
    "hgs:hgs:release"
    "hgs-git:hgs-git:git"
    "hgs-greeter:coastlinesec:release"
)
...
latest_tag() {
    git ls-remote --tags --refs --sort='-v:refname' https://github.com/CoastLineSec/HyprGlassShell.git |
        sed -n '1s|.*/v\{0,1\}||p'
}
