...
#
# Examples:
#   ./ppa-upload.sh hgs                    # Upload to questing + resolute (default)
#   ./ppa-upload.sh hgs 2                 # Native: questing ppa2, resolute ppa3 (auto +1 on second series)
#   ./ppa-upload.sh hgs --rebuild=2       # Rebuild with ppa2 (flag syntax)
#   ./ppa-upload.sh hgs-git               # Single package (both series)
#   ./ppa-upload.sh all                   # All packages (each to both series)
#   ./ppa-upload.sh hgs resolute          # 26.04 LTS only (same as "hgs hgs resolute")
#   ./ppa-upload.sh hgs questing          # 25.10 only
#   ./ppa-upload.sh hgs hgs resolute      # Explicit PPA name + one series (optional form)
#   ./ppa-upload.sh hgs hgs resolute 2    # One series + rebuild number
#   ./ppa-upload.sh distro/ubuntu/hgs hgs # Path-style (backward compatible)
...
# Shorthand: "hgs resolute" / "hgs questing" (package + series; PPA inferred — no need for "hgs hgs resolute")
...
if [ "$PPA_NAME" = "coastlinesec" ] || [ "$PPA_NAME" = "hgs" ] || [ "$PPA_NAME" = "hgs-git" ]; then
...
EOF
    ...
    # This branch should not be reached for HGS packages
    # All HGS packages (hgs, hgs-git, hgs-greeter) use lftp
    error "Unknown PPA: $PPA_NAME"
    error "HGS packages use lftp for upload. Supported PPAs: hgs, hgs-git, coastlinesec"
    exit 1
fi
...
if [ "$KEEP_BUILDS" = "false" ]; then
    ...
    hgssearch)
        ...
        ;;
    hgs)
        if [ -f "$PACKAGE_DIR/hgs-distropkg-amd64.gz" ]; then
            rm -f "$PACKAGE_DIR/hgs-distropkg-amd64.gz"
            REMOVED=$((REMOVED + 1))
        fi
        if [ -f "$PACKAGE_DIR/hgs-distropkg-arm64.gz" ]; then
            rm -f "$PACKAGE_DIR/hgs-distropkg-arm64.gz"
            REMOVED=$((REMOVED + 1))
        fi
        if [ -f "$PACKAGE_DIR/hgs-source.tar.gz" ]; then
            rm -f "$PACKAGE_DIR/hgs-source.tar.gz"
            REMOVED=$((REMOVED + 1))
        fi
        ;;
    hgs-git)
        # Remove git source directory binary
        if [ -d "$PACKAGE_DIR/hgs-git-repo" ]; then
            rm -rf "$PACKAGE_DIR/hgs-git-repo"
            REMOVED=$((REMOVED + 1))
        fi
        ;;
    hgs-greeter)
        # Remove downloaded source
        if [ -f "$PACKAGE_DIR/hgs-greeter-source.tar.gz" ]; then
            rm -f "$PACKAGE_DIR/hgs-greeter-source.tar.gz"
            REMOVED=$((REMOVED + 1))
        fi
        ;;
    esac
