...
Item {
    id: root
    readonly property var log: Log.scoped("HGSShellIPC")
    ...
    required property var hgsDashPopoutLoader
    required property var notepadSlideoutVariants
    required property var hyprKeybindsModalLoader
    required property var hgsBarRepeater
    ...
    function getPreferredBar(refPropertyName) {
        if (!root.hgsBarRepeater || root.hgsBarRepeater.count === 0)
            return null;

        const focusedScreenName = BarWidgetService.getFocusedScreenName();

        const loaders = Array.from({
            length: root.hgsBarRepeater.count
        }, (_, i) => root.hgsBarRepeater.itemAt(i));
    ...
    function launchDefaultMimeApp(appName, mimeType) {
        HGSService.sendRequest("mime.getDefault", {
            "mimeType": mimeType
        }, response => {
            if (response.error) {
    ...
    IpcHandler {
        ...
        function open(tab: string): string {
            ...
            const dash = root.hgsDashPopoutLoader.item;
        ...
        function close(): string {
            if (root.hgsDashPopoutLoader.item) {
                root.hgsDashPopoutLoader.item.dashVisible = false;
        ...
        function toggle(tab: string): string {
            if (root.hgsDashPopoutLoader.item?.dashVisible) {
                root.hgsDashPopoutLoader.item.dashVisible = false;
    ...
    IpcHandler {
        ...
        function toggleOverview(): string {
            if (!root.hyprlandOverviewLoader.item) {
    ...
    IpcHandler {
        function open(): string {
            PopoutService.openHGSLauncherV2();
            return "LAUNCHER_OPEN_SUCCESS";
        }

        function close(): string {
            PopoutService.closeHGSLauncherV2();
            return "LAUNCHER_CLOSE_SUCCESS";
        }

        function toggle(): string {
            PopoutService.toggleHGSLauncherV2();
        ...
        function openWith(mode: string): string {
            ...
            PopoutService.openHGSLauncherV2WithMode(mode);
    ...
    IpcHandler {
        function open(): string {
            return "WORKSPACE_RENAME_UNSUPPORTED";
        }
    ...
    IpcHandler {
        function listProfiles(): string {
            ...
            const activeId = SettingsData.getActiveDisplayProfile("hyprland");
