...
Item {
    ...
    Connections {
        ...
        function onLauncherSearchReady(query) {
            ...
            const clipboardBuiltInActive = activePluginId === "hgs_clipboard_search";
            ...
            const effectiveQuery = clipboardBuiltInActive && triggerMatch.pluginId === "hgs_clipboard_search" ? triggerMatch.query : trimmed;
    ...
    function setSearchQuery(query) {
        ...
        var filesInAll = searchMode === "all" && (SettingsData.hgsLauncherV2IncludeFilesInAll || SettingsData.hgsLauncherV2IncludeFoldersInAll);
    ...
    function setMode(mode, isAutoSwitch, fileTypeOverride, notPersist) {
        ...
        performSearch();
        var filesInAll = mode === "all" && (SettingsData.hgsLauncherV2IncludeFilesInAll || SettingsData.hgsLauncherV2IncludeFoldersInAll) && searchQuery.length > 0;
    ...
    function performSearch() {
        ...
        if (searchMode === "apps") {
            ...
            if (isCategoryFiltered) {
                ...
                // Also include core apps (HGS Settings etc.) that match this category
    ...
    function _performPluginPhase() {
        ...
        for (var i = 0; i < allPluginsOrdered.length; i++) {
            ...
            if (plugin.isBuiltIn && (plugin.id === "hgs_settings_search" || plugin.id === "hgs_clipboard_search"))
                continue;
            if (plugin.isBuiltIn) {
                var blItems = AppSearchService.getBuiltInLauncherItems(plugin.id, searchQuery);
    ...
    function performFileSearch() {
        ...
        var includeFiles = SettingsData.hgsLauncherV2IncludeFilesInAll;
        var includeFolders = SettingsData.hgsLauncherV2IncludeFoldersInAll;
    ...
    function transformBuiltInSearchItem(item, pluginId) {
        if (pluginId === "hgs_clipboard_search" || item.type === "clipboard")
    ...
    function clipboardSearchEnabledInAll() {
        return builtInLauncherVisibleInAll("hgs_clipboard_search") && ClipboardService.clipboardAvailable;
    }
    ...
    function appendSharedAllResults(allItems, query) {
        ...
        if (builtInLauncherVisibleInAll("hgs_settings_search")) {
            var settingsItems = AppSearchService.getBuiltInLauncherItems("hgs_settings_search", query);
            var settingsLimit = Math.min(settingsItems.length, 8);
            for (var i = 0; i < settingsLimit; i++) {
                settingsItems[i]._preScored = 890 - i;
                allItems.push(transformBuiltInSearchItem(settingsItems[i], "hgs_settings_search"));
            }
        }

        if (clipboardSearchEnabledInAll()) {
            var clipboardItems = AppSearchService.getBuiltInLauncherItems("hgs_clipboard_search", query);
            var clipboardLimit = Math.min(clipboardItems.length, 8);
            for (var j = 0; j < clipboardLimit; j++) {
                clipboardItems[j]._preScored = 840 - j;
                allItems.push(transformBuiltInSearchItem(clipboardItems[j], "hgs_clipboard_search"));
            }
        }
    }
    ...
    function buildDynamicSectionDefs(items) {
        ...
        for (var d = 0; d < baseDefs.length; d++) {
            var virtualId = baseDefs[d].id === "settings" ? "hgs_settings_search" : baseDefs[d].id === "clipboard" ? "hgs_clipboard_search" : "";
