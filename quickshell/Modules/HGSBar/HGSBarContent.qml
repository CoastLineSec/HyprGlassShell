...
import qs.Common
import qs.Modules.HGSBar.Widgets
import qs.Services
...
Item {
    ...
    Component {
        ...
        Clock {
            ...
            popoutTarget: hgsDashPopoutLoader.item ?? null
            parentScreen: barWindow.screen
            ...
            onClockClicked: {
                ...
                if (hgsDashPopoutLoader.item) {
                    const effectiveBarConfig = topBarContent.barConfig;
                    const barPosition = barWindow.axis?.edge === "left" ? 2 : (barWindow.axis?.edge === "right" ? 3 : (barWindow.axis?.edge === "top" ? 0 : 1));
                    if (hgsDashPopoutLoader.item.setBarContext) {
                        hgsDashPopoutLoader.item.setBarContext(barPosition, effectiveBarConfig?.bottomGap ?? 0);
                    }
                    if (hgsDashPopoutLoader.item.setTriggerPosition) {
                        ...
                        hgsDashPopoutLoader.item.setTriggerPosition(pos.x, pos.y, pos.width, section, barWindow.screen, barPosition, barWindow.effectiveBarThickness, effectiveBarConfig?.spacing ?? 4, effectiveBarConfig);
                    } else {
                        hgsDashPopoutLoader.item.triggerScreen = barWindow.screen;
                    }
                    PopoutManager.requestPopout(hgsDashPopoutLoader.item, 0, (effectiveBarConfig?.id ?? "default") + "-" + section + "-0");
                }
            }
        }
    }
        ...
        Media {
            ...
            onClicked: {
                ...
                if (hgsDashPopoutLoader.item) {
                    ...
                    PopoutManager.requestPopout(hgsDashPopoutLoader.item, 1, (effectiveBarConfig?.id ?? "default") + "-" + section + "-1");
                }
            }
        }
    }
        ...
        Weather {
            ...
            onClicked: {
                ...
                if (hgsDashPopoutLoader.item) {
                    ...
                    if (hgsDashPopoutLoader.item.setTriggerPosition) {
                        // For center section widgets, use center section bounds for HGSDash centering
                        let triggerPos, triggerWidth;
                        if (section === "center") {
                            const centerSection = barWindow.isVertical ? (barWindow.axis?.edge === "left" ? vCenterSection : vCenterSection) : hCenterSection;
                            if (centerSection) {
                                ...
                                if (barWindow.isVertical) {
                                    ...
                                    // For horizontal bars, use left edge (HGSPopout will center it)
                                    const centerGlobalPos = centerSection.mapToItem(null, 0, 0);
                                    triggerPos = centerGlobalPos;
                                    triggerWidth = centerSection.width;
                                }
                    ...
                    PopoutManager.requestPopout(hgsDashPopoutLoader.item, 3, (effectiveBarConfig?.id ?? "default") + "-" + section + "-3");
                }
            }
        }
    }
