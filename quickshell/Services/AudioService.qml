...
Singleton {
    ...
    property string wireplumberConfigPath: Paths.strip(StandardPaths.writableLocation(StandardPaths.ConfigLocation)) + "/wireplumber/wireplumber.conf.d/51-hgs-audio-aliases.conf"
...
`;
    ...
    function generateWireplumberConfig() {
        let config = "# Generated by HyprGlassShell - Audio Device Aliases\n";
    ...
    function playLoginSoundIfApplicable() {
        if (SettingsData.soundsEnabled && SettingsData.soundLogin && !notificationsAudioMuted) {
            ...
            // to prevent it from playing on every HGS restart during the session
            ...
            const loginFile = `${runtimeDir}/coastlinesec.login-${sessionId}`;
