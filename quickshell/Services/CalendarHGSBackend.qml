...
Item {
    id: root
    readonly property var log: Log.scoped("CalendarHGSBackend")
    ...
    Process {
        id: discoverProcess
        running: false
        command: ["sh", "-c", "s=\"${HGSCAL_SOCKET:-}\"; if [ -S \"$s\" ]; then echo \"$s\"; exit 0; fi; for f in \"${XDG_RUNTIME_DIR:-/tmp}\"/hgscal-*.sock /tmp/hgscal-*.sock; do [ -S \"$f\" ] || continue; p=$(basename \"$f\" .sock); p=${p#hgscal-}; if kill -0 \"$p\" 2>/dev/null; then echo \"$f\"; exit 0; fi; done"]
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (!root.connected) {
                    if (root.socketPath !== "")
                        root.log.info("hgscal socket gone, waiting for daemon");
                    requestSocket.connected = false;
                    subscribeSocket.connected = false;
                    root.socketPath = "";
                }
    ...
    function _applySocketPath(path) {
        ...
        if (changed)
            log.info("hgscal socket discovered:", path);
    ...
    HGSSocket {
        ...
        onConnectionStateChanged: {
            if (linkUp) {
                ...
                root.log.info("connected to hgscal:", root.socketPath);
            ...
            requestSocket.connected = false;
            subscribeSocket.connected = false;
            root.log.info("hgscal disconnected, rediscovering");
            if (root.enabled)
                discoverProcess.running = true;
        }
    ...
    function sendRequest(method, params, callback) {
        if (!connected) {
            if (callback)
                callback({
                    "error": "not connected to hgscal socket"
                });
            return;
        }
        const id = _nextId();
