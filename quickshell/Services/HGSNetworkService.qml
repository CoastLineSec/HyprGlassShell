...
Singleton {
    id: root
    readonly property var log: Log.scoped("HGSNetworkService")
    ...
    function checkHGSCapabilities() {
        ...
        networkAvailable = HGSService.capabilities.includes("network");
    ...
    function getState() {
        if (!networkAvailable)
            return;
        HGSService.sendRequest("network.getState", null, response => {
            if (response.result) {
    ...
    function updateState(state) {
        ...
        if (state.wifiNetworks || state.savedWifiNetworks) {
            const hasSavedWifiState = HGSService.apiVersion >= savedWifiStateApiVersion && Array.isArray(state.savedWifiNetworks);
    ...
    function connectToSpecificWiredConfig(uuid) {
        ...
        HGSService.sendRequest("network.ethernet.connect.config", params, response => {
            if (response.error) {
    ...
    function scanWifi() {
        ...
        HGSService.sendRequest("network.wifi.scan", params, response => {
            isScanning = false;
            if (response.error) {
    ...
    function connectToWifi(ssid, password = "", username = "", anonymousIdentity = "", domainSuffixMatch = "", hidden = false) {
        ...
        HGSService.sendRequest("network.wifi.connect", params, response => {
            if (response.error) {
    ...
    function disconnectWifi() {
        ...
        HGSService.sendRequest("network.wifi.disconnect", params, response => {
            if (response.error) {
    ...
    function submitCredentials(token, secrets, save) {
        log.debug("submitCredentials: networkAvailable=" + networkAvailable + " apiVersion=" + HGSService.apiVersion);

        if (!networkAvailable || HGSService.apiVersion < 7) {
            log.warn("submitCredentials: Aborting - networkAvailable=" + networkAvailable + " apiVersion=" + HGSService.apiVersion);
            return;
        }
        ...
        HGSService.sendRequest("network.credentials.submit", params, response => {
            if (response.error) {
    ...
    function cancelCredentials(token) {
        ...
        HGSService.sendRequest("network.credentials.cancel", params, response => {
            if (response.error) {
    ...
    function forgetWifiNetwork(ssid) {
        if (!networkAvailable)
            return;
        forgetSSID = ssid;
        HGSService.sendRequest("network.wifi.forget", {
            ssid: ssid
        }, response => {
            if (response.error) {
    ...
    function toggleWifiRadio() {
        if (!networkAvailable || wifiToggling)
            return;
        wifiToggling = true;
        HGSService.sendRequest("network.wifi.toggle", null, response => {
            wifiToggling = false;
    ...
    function enableWifiDevice() {
        if (!networkAvailable)
            return;
        HGSService.sendRequest("network.wifi.enable", null, response => {
            if (response.error) {
    ...
    function setNetworkPreference(preference) {
        ...
        HGSService.sendRequest("network.preference.set", {
    ...
    function toggleNetworkConnection(type) {
        ...
        if (type === "ethernet") {
            if (ethernetConnected) {
                HGSService.sendRequest("network.ethernet.disconnect", null, null);
            } else {
                HGSService.sendRequest("network.ethernet.connect", null, null);
            }
        }
    }
    ...
    function disconnectEthernetDevice(deviceName) {
        if (!networkAvailable)
            return;
        HGSService.sendRequest("network.ethernet.disconnect", {
            device: deviceName
        }, null);
    }
    ...
    function fetchWiredNetworkInfo(uuid) {
        ...
        HGSService.sendRequest("network.ethernet.info", {
            uuid: uuid
        }, response => {
    ...
    function fetchNetworkInfo(ssid) {
        ...
        HGSService.sendRequest("network.info", {
            ssid: ssid
        }, response => {
    ...
    function refreshVpnProfiles() {
        if (!vpnAvailable)
            return;
        HGSService.sendRequest("network.vpn.profiles", null, response => {
            if (response.result) {
    ...
    function refreshVpnActive() {
        if (!vpnAvailable)
            return;
        HGSService.sendRequest("network.vpn.active", null, response => {
            if (response.result) {
    ...
    function connectVpn(uuidOrName, singleActive = false) {
        ...
        HGSService.sendRequest("network.vpn.connect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                pendingVpnUuid = "";
                vpnBusyStartTime = 0;
    ...
    function disconnectVpn(uuidOrName) {
        ...
        HGSService.sendRequest("network.vpn.disconnect", params, response => {
            if (response.error) {
                vpnIsBusy = false;
                vpnBusyStartTime = 0;
    ...
    function disconnectAllVpns() {
        ...
        HGSService.sendRequest("network.vpn.disconnectAll", null, response => {
            if (response.error) {
                vpnIsBusy = false;
    ...
    function setWifiAutoconnect(ssid, autoconnect) {
        if (!networkAvailable || HGSService.apiVersion <= 13)
            return;
        const params = {
            ssid: ssid,
        ...
        HGSService.sendRequest("network.wifi.setAutoconnect", params, response => {
            if (response.error) {
