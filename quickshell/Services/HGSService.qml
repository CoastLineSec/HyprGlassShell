...
Singleton {
    id: root
    readonly property var log: Log.scoped("HGSService")

    property bool hgsAvailable: false
    ...
    property string updateCommand: "hgs update"
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (helper.includes("paru")) {
                    checkHgsPackage.helper = "paru";
                    checkHgsPackage.running = true;
                } else if (helper.includes("yay")) {
                    checkHgsPackage.helper = "yay";
                    checkHgsPackage.running = true;
                } else {
                    updateCommand = "hgs update";
    ...
    Process {
        ...
        command: ["sh", "-c", "pacman -Qi hgs-shell-git 2>/dev/null || pacman -Qi hgs-shell-bin 2>/dev/null"]
        running: false

        stdout: StdioCollector {
            onStreamFinished: {
                if (text.includes("hgs-shell-git")) {
                    updateCommand = checkHgsPackage.helper + " -S hgs-shell-git";
                } else if (text.includes("hgs-shell-bin")) {
                    updateCommand = checkHgsPackage.helper + " -S hgs-shell-bin";
                } else {
                    updateCommand = "hgs update";
                }
                checkingUpdateCommand = false;
    ...
    Process {
        ...
        onExited: exitCode => {
            if (exitCode === 0) {
                root.hgsAvailable = true;
                connectSocket();
            } else {
                root.hgsAvailable = false;
            }
        }
    }

    function connectSocket() {
        if (!hgsAvailable || isConnected || isConnecting) {
            return;
        }
    ...
    function handleSubscriptionEvent(response) {
        if (response.error) {
            if (response.error.includes("unknown method") && response.error.includes("subscribe")) {
                if (!shownOutdatedError) {
                    ...
                    ToastService.showError(I18n.tr("HGS out of date"), I18n.tr("To update, run the following command:"), updateCommand);
        ...
        if (service === "server") {
            ...
            if (apiVersion < expectedApiVersion) {
                ToastService.showError(I18n.tr("HGS server is outdated (API v%1, expected v%2)").arg(apiVersion).arg(expectedApiVersion));
            }
    ...
    function sendRequest(method, params, callback) {
        if (!isConnected) {
            log.warn("HGSService.sendRequest: Not connected, method:", method);
            if (callback) {
                callback({
                    "error": "not connected to HGS socket"
                });
            }
            return;
        }
        ...
        if (method.startsWith("clipboard")) {
            ...
            log.debug("HGSService.sendRequest: Sending request id=" + id + " method=" + method);
        }
    ...
    function listPlugins(callback) {
        ...
        if (callback)
            callback({
                "error": "Plugin registry is disabled in HyprGlassShell"
            });
    }

    function listInstalled(callback) {
        installedPlugins = [];
        installedPluginsReceived([]);
        if (callback)
            callback({
                "result": []
            });
    ...
    function install(pluginName, callback) {
        if (callback)
            callback({
                "error": "Plugin installation is disabled in HyprGlassShell"
            });
    }
    ...
    function uninstall(pluginName, callback) {
        if (callback)
            callback({
                "error": "Plugin management is disabled in HyprGlassShell"
            });
    }
    ...
    function update(pluginName, callback) {
        if (callback)
            callback({
                "error": "Plugin updates are disabled in HyprGlassShell"
            });
    }
