...
Singleton {
    ...
    function refreshCount() {
        Proc.runCommand("trash-count", ["hgs", "trash", "count"], (output, exitCode) => {
            if (exitCode !== 0) {
    ...
    function trashPath(path, callback) {
        ...
        Proc.runCommand(null, ["hgs", "trash", "put", path], (output, exitCode) => {
            const ok = exitCode === 0;
            if (!ok)
    ...
    function emptyTrash() {
        Proc.runCommand("trash-empty", ["hgs", "trash", "empty"], (output, exitCode) => {
            if (exitCode !== 0)
