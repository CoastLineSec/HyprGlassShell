...
Frame {
    ...
    function modeChoices() {
        const choices = [
            { label: qsTr("Preferred"), value: "preferred" },
            { label: qsTr("Highest refresh rate"), value: "highrr" },
            { label: qsTr("Highest resolution"), value: "highres" }
        ];
        const seen = { preferred: true, highrr: true, highres: true };
        const modes = root.observedOutput && Array.isArray(
            root.observedOutput.modes
        ) ? root.observedOutput.modes : [];
        for (const mode of modes) {
            if (!mode || !mode.width || !mode.height)
                continue;
            const value = root.explicitMode(
                mode.width,
                mode.height,
                mode.refreshRate,
                mode.managedMode
            ...
            choices.push({
                label: refresh > 0
                    ? qsTr("%1 × %2 at %3 Hz")
                        ...
                        .arg(refresh.toFixed(2).replace(/0+$/, "").replace(/\.$/, ""))
                    : qsTr("%1 × %2").arg(mode.width).arg(mode.height),
                value: value
            });
        }
        const current = root.output ? String(root.output.mode || "") : "";
        if (current && !seen[current])
            choices.push({ label: qsTr("Custom (%1)").arg(current), value: current });
        return choices;
    }
    ...
    function scaleChoices() {
        const choices = [
            ...
            { label: qsTr("100%"), value: 1 },
            { label: qsTr("125%"), value: 1.25 },
            { label: qsTr("150%"), value: 1.5 },
            { label: qsTr("175%"), value: 1.75 },
            { label: qsTr("200%"), value: 2 }
        ];
    ...
    function mirrorChoices() {
        const choices = [{ label: qsTr("Not mirrored"), value: "" }];
        if (!Array.isArray(root.allOutputs))
            return choices;
        for (const candidate of root.allOutputs) {
            if (!candidate || !root.output
                    || candidate.id === root.output.id
    ...
    readonly property bool automaticPosition: root.output
        && typeof root.output.position === "string"
        && root.output.position.indexOf("auto") === 0
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            ColumnLayout {
                ...
                Label {
                    Layout.fillWidth: true
                    text: root.observedOutput
                        ? [root.observedOutput.make, root.observedOutput.model]
                            .filter(value => value && String(value).length > 0)
                            .join(" ") || qsTr("Connected display")
                        : qsTr("Saved display")
        ...
        GridLayout {
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    currentIndex: root.indexForValue(
                        root.availableModes,
                        root.output ? root.output.mode : "preferred"
                    )
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    valueRole: "value"
                    currentIndex: root.indexForValue(
                        root.availableScales,
                        root.output ? root.output.scale : 1
                    )
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    Layout.fillWidth: true
                    model: [
                        { label: qsTr("Landscape"), value: 0 },
                        { label: qsTr("Portrait right"), value: 1 },
                        { label: qsTr("Landscape flipped"), value: 2 },
                        { label: qsTr("Portrait left"), value: 3 },
                        { label: qsTr("Flipped"), value: 4 },
                        { label: qsTr("Flipped portrait right"), value: 5 },
                        { label: qsTr("Flipped landscape"), value: 6 },
                        { label: qsTr("Flipped portrait left"), value: 7 }
                    ]
                    textRole: "label"
                    valueRole: "value"
                    currentIndex: root.indexForValue(
                        model,
                        root.output ? Number(root.output.transform) : 0
                    )
                    enabled: root.controlsEnabled
        ...
        RowLayout {
            ...
            SpinBox {
                ...
                onValueModified: root.updateField(
                    "position",
                    "%1x%2".arg(value).arg(root.positionPart(1))
                )
            }
            ...
            SpinBox {
                ...
                onValueModified: root.updateField(
                    "position",
                    "%1x%2".arg(root.positionPart(0)).arg(value)
                )
            }
        ...
        Button {
            ...
            text: checked ? qsTr("Hide advanced settings")
        ...
        GridLayout {
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    valueRole: "value"
                    currentIndex: root.indexForValue(
                        root.availableMirrors,
                        root.output ? root.output.mirror : ""
                    )
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    Layout.fillWidth: true
                    model: [
                        { label: qsTr("8-bit"), value: 8 },
                        { label: qsTr("10-bit"), value: 10 }
                    ]
                    textRole: "label"
                    valueRole: "value"
                    currentIndex: root.output && root.output.bitdepth === 10
                        ? 1 : 0
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    Layout.fillWidth: true
                    model: [
                        { label: qsTr("Use compositor default"), value: -1 },
                        { label: qsTr("Off"), value: 0 },
                        { label: qsTr("On"), value: 1 },
                        { label: qsTr("Fullscreen only"), value: 2 },
                        {
                    ...
                    currentIndex: root.indexForValue(
                        model,
                        root.output ? Number(root.output.vrr) : -1
                    )
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    model: [
                        { label: qsTr("Automatic"), value: "auto" },
                        { label: qsTr("sRGB"), value: "srgb" },
                        { label: qsTr("Wide gamut"), value: "wide" },
                        { label: qsTr("From display (EDID)"), value: "edid" },
                        { label: qsTr("HDR"), value: "hdr" },
                        { label: qsTr("HDR from display"), value: "hdredid" },
                        { label: qsTr("DCI-P3"), value: "dcip3" },
                        { label: qsTr("Display P3"), value: "dp3" },
                        { label: qsTr("Adobe RGB"), value: "adobe" }
                    ]
                    textRole: "label"
                    valueRole: "value"
                    currentIndex: root.indexForValue(
                        model,
                        root.output ? root.output.cm : "auto"
                    )
                    enabled: root.controlsEnabled
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    Layout.fillWidth: true
                    model: [
                        { label: qsTr("Automatic"), value: -1 },
                        { label: qsTr("Unsupported"), value: 0 },
                        { label: qsTr("Supported"), value: 1 }
                    ]
                    textRole: "label"
                    valueRole: "value"
                    currentIndex: root.indexForValue(
                        model,
                        root.output ? Number(root.output.supportsWideColor) : -1
                    )
                    enabled: root.controlsEnabled

                    onActivated: root.updateField(
                        "supportsWideColor",
                        currentValue
                    )
                }
            ...
            ColumnLayout {
                ...
                ComboBox {
                    ...
                    currentIndex: root.indexForValue(
                        model,
                        root.output ? Number(root.output.supportsHdr) : -1
                    )
                    enabled: root.controlsEnabled
        ...
        RowLayout {
            ...
            Label {
                Layout.fillWidth: true
                text: root.existingRecord
                    ? qsTr("Advanced values not shown here are preserved exactly for this saved display.")
                    : qsTr("This newly connected display starts from safe advanced defaults.")
                color: root.palette.placeholderText
                font.pixelSize: 11
                wrapMode: Text.Wrap
            }
